...
def get_most_common_word_weights(trainfile):
    ...
    """
    weights = defaultdict(float)
    # Put a default weight of 0.5 to nouns.
    weights[('NOUN', OFFSET)] = 0.7

    tag_word_counts = get_tag_word_counts(trainfile)
    for tag in tag_word_counts:
        for word in tag_word_counts[tag]:
            sorted_tags = sorted([(tag_word_counts[t][word], t) for t in tag_word_counts], reverse = True)[0][1]
            weights[(sorted_tags, word)] = 1.0
