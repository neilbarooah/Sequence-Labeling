...
class BiLSTM(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim, embeddings=None):
        ...
        self.lstm = nn.LSTM(input_size=embedding_dim, hidden_size=hidden_dim//2, bidirectional=True)
    ...
    def forward(self, sentence):
        ...
        """
        self.hidden = self.init_hidden()
        word_to_embed = self.word_embeds(sentence).view(len(sentence), 1, self.embedding_dim)
        output, new_hidden_state = self.lstm(word_to_embed, self.hidden)
        self.hidden = new_hidden_state
        tags = self.hidden2tag(output).view(len(sentence), -1)
        return tags
    ...
    def predict(self, sentence):
        ...
        probabilities = softmax_layer(lstm_feats)
        index = argmax(probabilities)
        tags = []
        for idx in index:
            tags.append(self.ix_to_tag[idx])
        return tags
...
class BiLSTM_CRF(BiLSTM):
    ...
    def forward_alg(self, feats):
        ...
        for feature in feats:
            ...
            for tag in range(self.tagset_size):
                emit_score = feature[tag].view(1, -1).expand(1, self.tagset_size)
                trans_score = self.transitions[tag].view(1, -1)
                next_tag_var = prev_scores + trans_score + emit_score
                score.append(log_sum_exp(next_tag_var))

            prev_scores = torch.cat(score).view(1, -1)
        last_word = prev_scores + self.transitions[self.tag_to_ix[END_TAG]]
        alpha = log_sum_exp(last_word)
        return alpha
    ...
    def score_sentence(self,feats, gold_tags):
        ...
        score += self.transitions[self.tag_to_ix[END_TAG], tags[-1].data[0]]
        return score
    ...
    def neg_log_likelihood(self, lstm_feats, gold_tags):
        ...
        self.hidden = self.init_hidden()
        score = self.forward_alg(lstm_feats)
